...
public class ConversationEnrichmentStep implements PipelineStep {
    ...
    private TopicEnrichment createBasicTopic(Map<?, ?> topicMap, List<SlackMessage> messages) {
        ...
        LocalDateTime deadline = parseDeadline(extractStringValue(topicMap, "deadline", null));
        ...
        try {
            ...
            try {
                ...
                if (deadlineStr.matches("\\d{4}-\\d{2}-\\d{2}")) {
                    return LocalDateTime.parse(deadlineStr + "T00:00:00");
                }
            } catch (Exception e2) {
                logger.debug("Failed to parse deadline as date-only '{}': {}", deadlineStr, e2.getMessage());
            }
            logger.debug("Failed to parse deadline '{}': {}", deadlineStr, e.getMessage());
            return null;
        }
    }
