...
public class RelatedUsersEnrichmentProcessor implements MessageProcessor {
    ...
    public ProcessingResult process(IngestionProcessingContext context) {
        ...
        logger.debug("Enriching related users for messageId: {}, channelId: {}", 
                    context.getMessageId(), ingestionEvent.getMessage().getChannelId());
        ...
        try {
            ...
            logger.info("Successfully enriched and published {} related users for messageId: {}, " +
                       "userChannelEventId: {}", 
                       relatedUsers.size(), context.getMessageId(), publishedMessageId);
            ...
            if (userId != null && !userId.trim().isEmpty() && !userId.equals(senderUserId)) {
                ...
                try {
                    ...
                    if (userOpt.isPresent()) {
                        ...
                        logger.debug("Found user in database: userId={}, userName={}", userId, userName);
                    } else {
                        logger.debug("User not found in database: userId={}, using userId as username", userId);
                    }
                } catch (Exception e) {
